...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String player2 = readNonEmpty(scanner, "Spieler 2 Name: ", player1);
        ...
        for (ShipType type : game.getFleetDefinition()) {
            for (int i = 1; i <= type.getCount(); i++) {
                while (true) {
                    try {
                        clearScreenHint();
                        System.out.println("Spieler: " + game.getActivePlayerName());
                        System.out.println(grid.renderOwnBoard());
    ...
    private static String readNonEmpty(Scanner scanner, String prompt, String forbidded) {
        while (true) {
            System.out.print(prompt);
            String value = scanner.nextLine();
            if (value != null && !value.trim().isEmpty()) {
                if (forbidded != null && forbidded.equals(value)) {
                    System.out.println("Doppelte Namen sind nicht erlaubt. Wähle einen anderen!");
                    continue;
                }
                return value.trim();
            }
